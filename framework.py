...
def text_to_hidden_state_mapping() -> None:
...
def step_game(
    current_room_desc: str,
    current_quest_desc: str,
    action_index: int,
    object_index: int,
    ...
    if command_is_valid[current_room_index, action_index, object_index] == 1:
        ...
        if (actions[action_index] == quest_actions[quest_index]) and (
            ...
            if DEBUG:
                print(
                    f"Finish quest: {current_quest_desc} at Room {current_room_desc} with command actions[action_index] {objects[object_index]}"
                )
...
# start a new game
def new_game() -> tuple[str, str, bool]:
    global STEP_COUNT
    STEP_COUNT = 0
    ...
    if DEBUG:
        ...
        print(f"Start Room {room_index}: {room_name}. {room_desc}")
        print("Start quest: {quest_desc}")
